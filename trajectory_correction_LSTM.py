# file for training the LSTM model for predicting flow errors of the analytical model
...
test_model = ResidualLSTM().to(device)
criterion = torch.nn.MSELoss()
optimizer = torch.optim.Adam(test_model.parameters(), lr=1e-3)

state_dict = torch.load('lstm_residual_model_tmp.pth')
...
test_analytical = test_inputs[batchID][:len(test_time),3] / 1e9
test_residuals = test_result[batchID].reshape(-1)[:len(test_time)] / 1e9
test_sim_residuals = (test_batch_targets[batchID]).cpu().numpy().reshape(-1)[:len(test_time)] / 1e9

test_output_lstm = test_analytical + test_residuals
test_output_sim = test_analytical + test_sim_residuals
...
plt.plot(test_time, test_analytical,
...
fig_residuals = plt.figure("residual prediction")
...
plt.plot(test_time, test_residuals,
         color='blue', linewidth = 2, linestyle='-',
         label = 'Commanded Flowrate')
plt.plot(test_time, test_sim_residuals,
         color='red', linewidth = 2, linestyle='-',
         label = 'Simulation Prediction')
...
prior_error = np.sqrt(np.mean(((test_output_sim) - (test_analytical)) ** 2))
lstm_prior_error = np.sqrt(np.mean(((test_output_sim) - (test_output_lstm)) ** 2))

print('Analytical RMSE:', prior_error)
print("Analytical+LSTM RMSE:", lstm_prior_error)
...
ax.hist((test_output_sim - test_analytical)*1e9, bins=40, alpha=0.5, color='red', label='Analytical Error', density=True)
ax.hist((test_output_sim - test_output_lstm)*1e9, bins=40, alpha=0.5, color='blue', label='Analytical+LSTM Error', density=True)
...
error_residuals = plt.figure("error legend")
error_residuals.legend(ax.get_legend_handles_labels()[0],ax.get_legend_handles_labels()[1])
...
#
# fig_residuals.savefig('residuals.png',bbox_inches='tight',dpi=600)
# leg_residuals.savefig('residuals_leg.png',bbox_inches='tight',dpi=600)
#
